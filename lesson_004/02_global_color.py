...
def user_input():
    print('*'*10, 'НАЧАЛО', '*'*10, )
    for input_users in colors.items():
    ...
    while True:
        if input_users in colors:
            user_input = colors[input_users]
            print(colors[input_users]['sd_name'])
            # TODO здесь нужно выполнить return выбранного цвета
        else:
            print('Вы ввели не корректный номер:')
            continue  # TODO эта строка лишняя
...
# Распоковка название цветов произошла, а дальше как произвести отрисовку фигур не пойму.
